...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, onLogout }) => {
  ...
  return (
    <>
      <aside className="hidden w-64 shrink-0 flex-col border-r border-slate-700 bg-slate-900 p-6 md:flex">
        ...
        <nav className="space-y-1">
          ...
          >
            <IconLibrary className={`w-5 h-5 ${activeTab === 'templates' ? 'text-green-400' : ''}`} />
            Templates
          </button>
          <button
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 flex h-16 items-stretch border-t border-slate-700/70 bg-slate-900/95 px-2 backdrop-blur md:hidden">
        ...
        <button type="button" onClick={() => onTabChange('tailscale')} className={mobileItemClass(activeTab === 'tailscale')}>
          <IconNetwork className="h-5 w-5" />
          <span className="truncate">Tailnet</span>
        </button>
        <button type="button" onClick={() => onTabChange('settings')} className={mobileItemClass(activeTab === 'settings')}>
          <IconSettings className="h-5 w-5" />
          <span className="truncate">Settings</span>
        </button>
        {onLogout && (
          <button type="button" onClick={onLogout} className={mobileItemClass(false)}>
